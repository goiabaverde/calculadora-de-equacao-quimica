    ...
    resolver_eq(eq){
        ...
        for(var i = 0; i < eq.length; i++ ){
            if(isCapitalLetter(eq[i])){
                ...
                if(incognita.length > 1 || incognita == undefined){
        ...
        if(parts[local_incognita].split('+').length > 1){
            ...
            for(var n = 0; n < incognita_member.length; n++){
                if(incognita_member[n].includes(incognita) == false){
                    var term = incognita_member[n].trim()
                    incognita_member.splice(n,1)
...
equacoes.forEach(equacao=>{
    coeficientes.resolver_eq(equacao)
})
